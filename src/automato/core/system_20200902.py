...
def _reset():
  global handler_on_message, handler_on_entry_load, handler_on_entry_install, handler_on_loaded, handler_on_initialized
  handler_on_message = []
...
def _stats_show():
  ...
  logging.debug(('SYSTEM> DEBUG TIMINGS\n  total: {total}min\n' +
    ...
    schits = scripting_js.script_eval_cache_hits, sctotal = scripting_js.script_eval_cache_hits + scripting_js.script_eval_cache_miss, scperc = round(scripting_js.script_eval_cache_hits * 100 / (scripting_js.script_eval_cache_hits + scripting_js.script_eval_cache_miss)) if (scripting_js.script_eval_cache_hits + scripting_js.script_eval_cache_miss) > 0 else 0, scsize = len(scripting_js.script_eval_cache), scskip =  scripting_js.script_eval_cache_skipped, scdisabled = scripting_js.script_eval_cache_disabled,
...
def _on_events_passthrough_listener(source_entry, entry, eventname, eventdata):
  _entry_event_publish_and_invoke_listeners(source_entry, '#events_passthrough', eventname, eventdata['params'])
...
def message_payload_serialize(payload):
  return str(payload if not isinstance(payload, dict) else {x:context[x] for x in sorted(context)})
      ...
      if 'events' in self.definition:
        for eventname in self.definition['events']:
          if ('listen_all_events' in config and config['listen_all_events']) or (eventname in events_listeners and ("*" in events_listeners[eventname] or self.entry.id in events_listeners[eventname])):
            ...
            if event:
              _s = _stats_start()
              eventdata = _entry_event_publish(self.entry, self.topic_rule, event['name'], event['params'])
...
def _on_mqtt_message(topic, payload_source, payload, qos, retain, matches, timems):
  ...
  for pm in m.publishedMessages():
    ...
    for eventname in events:
      _entry_event_invoke_listeners(pm.entry, eventname, events[eventname])
...
def _entry_event_publish(entry, entry_topic, eventname, params):
  """
  ...
  params_key = json.dumps({k:params[k] for k in event_params_keys if params and k in params})
  ...
  with events_published_lock:
    ...
    if not isinstance(params, dict) or 'temporary' not in params or not params['temporary']:
      ...
      events_published[eventname][entry.id][params_key][entry_topic] = events_published[eventname][entry.id][params_key]["*"] = events_published[eventname]['*'] = {
        'time': time(),
        'params': params,
        'changed_params': changed_params,
      }
  
  return { 'params': params, 'changed_params': changed_params }
...
def _entry_event_invoke_listeners(entry, eventname, eventdata):
  """
...
def _entry_event_publish_and_invoke_listeners(entry, entry_topic, eventname, params):
  eventdata = _entry_event_publish(entry, entry_topic, eventname, params)
  _entry_event_invoke_listeners(entry, eventname, eventdata)
...
def _entry_actions_install(entry):
  ...
  for topic in entry.definition['subscribe']:
    if 'actions' in entry.definition['subscribe'][topic]:
      for action in entry.definition['subscribe'][topic]['actions']:
        if not action in entry.actions:
...
def entry_support_action(entry, action):
  ...
  with events_published_lock:
    ...
    if eventname in events_published and entry_id in events_published[eventname]:
      for params_key in events_published[eventname][entry_id]:
        if topic is None or topic in events_published[eventname][entry_id]:
